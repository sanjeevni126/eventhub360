...
function AssetManagement() {
  ...
  const fetchAssets = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/assets', { headers: { 'Authorization': `Bearer ${token}` } });
      if (response.ok) {
  ...
  const handleAllocate = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5001/api/assets/allocate`, {
        method: 'POST',
        headers: { 
  ...
  const handleRevoke = async (asset) => {
    ...
    try {
      const response = await fetch(`http://localhost:5001/api/assets/${asset.id}/return`, {
        method: 'POST',
