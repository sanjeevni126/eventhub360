...
function LeaveManagement() {
  ...
  const fetchLeaveTypes = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/leaves/types', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchBalances = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/leaves/balances', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      const response = await fetch(`http://localhost:5001/api/leaves/${id}/status`, {
        method: 'PUT',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
